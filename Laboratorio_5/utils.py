...
def instanceCost(weight, instance):
    lenght = len(weight)
    h0 = 1/(1+((math.e)**(-scalarProduct(instance,weight))))
    if (instance[lenght-1]== 1):
        return -math.log10(h0)
    elif (instance[lenght-1]== 0):
        return -math.log10(1-h0)

def costFunction(weight, data):
    lenght = len(weight)
    cost=0
    for instance in data:
        cost += instanceCost(weight,instance)
    return cost/lenght

def descentByGradient(weight, data, a):
    newWeight = []
    lenght = len(weight)
    for j in range(lenght):
        sum = 0
        for instance in data:
            sum += ((1/(1+((math.e)**(-scalarProduct(instance,weight)))))-instance[lenght-1])