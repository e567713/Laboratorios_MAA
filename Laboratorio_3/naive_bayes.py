	...
	def __init__(self, data, attributes, target_attr):
		...
		for instance in data:
			...
			for attribute in attributes:
				...
				if instance[attribute].decode() in self.values_frecuency[attribute]:
					if instance[target_attr].decode() == 'YES':
						self.values_frecuency[attribute][instance[attribute].decode()]['YES'] += 1
					elif instance[target_attr].decode() == 'NO':
						self.values_frecuency[attribute][instance[attribute].decode()]['NO'] += 1
				
				# En caso contrario, primero se crea la entrada de ese valor, para luego
				# colocarle el contador en 1 en dónde corresponda.
				else:
					self.values_frecuency[attribute][instance[attribute].decode()]= {'YES':0,'NO':0}
					if instance[target_attr].decode() == 'YES':
						self.values_frecuency[attribute][instance[attribute].decode()]['YES'] = 1
					elif instance[target_attr].decode() == 'NO':
						# print (attribute)
